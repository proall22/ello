...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        <div className={cn("absolute inset-0 -z-10 backdrop-blur-[12px] transition-opacity duration-[350ms] ease-out", (scrolled || menuOpen) ? "opacity-100" : "opacity-0")} />
        <div className="container mx-auto px-6 flex items-center justify-between">
          {/* LOGO */}
          <Link href="/" className="hover:scale-105 transition-transform duration-300 relative z-[101] pointer-events-auto" onClick={() => setMenuOpen(false)}>
            <img
              src="/assets/logo/logoEllolCreative.png"
              alt="Ellol Digital Creatives"
              className="h-14 md:h-20 w-auto object-contain mix-blend-screen relative z-10 opacity-90 hover:opacity-100"
            />
          </Link>
          ...
          >
            <span className={cn("block w-10 h-0.5 bg-white transition-all duration-300", menuOpen ? "rotate-45 translate-y-3" : "group-hover:scale-110")} />
            <span className={cn("block w-10 h-0.5 bg-white transition-all duration-300", menuOpen ? "opacity-0" : "group-hover:w-8")} />
            <span className={cn("block w-10 h-0.5 bg-white transition-all duration-300", menuOpen ? "-rotate-45 -translate-y-3" : "group-hover:scale-110")} />
          </button>
        </div>
      </header>
